...
function App() {
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '50px auto', fontFamily: 'system-ui', textAlign: 'center', padding: '0 15px' }}>
      
      {/* Injecting CSS animation for the spinner */}
      <style>
        {`
          @keyframes spin {
      ...
      {/* Conditional Rendering for the Cold Start */}
      {isServerWaking ? (
        <div style={{ padding: '50px 0' }}>
          ...
          <p style={{ color: '#475569', fontSize: '18px', fontWeight: '500', margin: '0 0 8px 0' }}>
            Waking up the engine...
          </p>
          <p style={{ color: '#94a3b8', fontSize: '14px', margin: 0 }}>
            (This takes ~30 seconds on the first load)
          </p>
        </div>
      ) : (
        <>
          ...
          <button 
            ...
            style={{ padding: '15px 30px', fontSize: '18px', fontWeight: 'bold', backgroundColor: loading ? '#94a3b8' : '#2563eb', color: 'white', border: 'none', borderRadius: '5px', cursor: loading ? 'not-allowed' : 'pointer', width: '100%', transition: '0.2s' }}
          >
            {loading ? "Calculating..." : "Simulate Matchup"}
          </button>

          {error && <p style={{ color: '#ef4444', marginTop: '20px', fontWeight: 'bold' }}>{error}</p>}
